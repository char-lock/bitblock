...
class BlockCache(object):
    ...
    def _read_block(self) -> Dict:
        ...
        _block["prevBlock"]: str = endian_swap(_cache.read(32)).hex()
        _block["merkleRoot"]: str = endian_swap(_cache.read(32)).hex()
        ...
        _cache.seek(0, 0)
        _block["hash"]: str = endian_swap(sha256(
            sha256(_cache.read(80)).digest()
        ).digest()).hex()
